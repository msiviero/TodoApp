...
public class TodoControllerTests
{
    ...
    {
        var mockService = new Mock<ITodoService>();
        mockService.Setup(s => s.Get(It.Is<string>(x => x == "abc"))).Returns(Todo.Create("abc", new DateTime(2023, 1, 1), new DateTime(2023, 1, 1),"Buy milk", false));

        var underTest = new TodoController(mockService.Object);
        var result = (OkObjectResult?)underTest.GetOne("abc").Result;

        Assert.Equal(Todo.Create("abc", new DateTime(2023, 1, 1), new DateTime(2023, 1, 1),"Buy milk", false), result?.Value);
